...
if (isset($_POST['resend_abstract']) && $_POST['resend_abstract'] === '1') {
  // Clear buffer to ensure clean JSON output (prevents PHP notices from breaking JSON)
  ob_clean();
  header('Content-Type: application/json');

  $to = "abstract@icpm.ae";
  $refId = isset($_SESSION['id']) ? $_SESSION['id'] : 'Unknown';
  $authorName = isset($_SESSION['name']) ? $_SESSION['name'] : 'Unknown';
  $authorEmail = isset($_SESSION['email']) ? $_SESSION['email'] : 'Unknown';
  
  // Try to get file from session first
  ...
  // If session path is invalid or missing, try database
  if (!$path || !is_file($path)) {
      if (isset($_SESSION['id'])) {
          $stmt = mysqli_prepare($con, "SELECT abstract_filename, abstract_blob, postertitle FROM users WHERE id=?");
          mysqli_stmt_bind_param($stmt, "i", $_SESSION['id']);
          mysqli_stmt_execute($stmt);
          mysqli_stmt_bind_result($stmt, $dbName, $dbBlob, $dbTitle);
          if (mysqli_stmt_fetch($stmt)) {
              if ($dbBlob) {
                  $name = $dbName ? $dbName : "abstract_ref_{$refId}.pdf";
                  $tempDir = sys_get_temp_dir();
                  $path = $tempDir . DIRECTORY_SEPARATOR . 'resend_' . uniqid() . '_' . $name;
                  file_put_contents($path, $dbBlob);
                  $isTempFile = true;
              }
          }
  ...
  if (!$path || !is_file($path)) {
    echo json_encode(array('ok'=>false,'error'=>'No abstract file available to resend. Please upload your abstract first.'));
    exit;
  }

  $posterTitle = !empty($dbTitle) ? $dbTitle : (isset($_SESSION['postertitle']) ? $_SESSION['postertitle'] : 'Not Provided');

  $subj = "Abstract Resend: Ref #$refId - $authorName";
  $body = "Dear Committee Members,\n\n";
  $body .= "Please find attached the abstract submission for the ICPM 2026 conference.\n\n";
  $body .= "Reference Number: $refId\n";
  $body .= "Author Name: $authorName\n";
  $body .= "Author Email: $authorEmail\n";
  $body .= "Poster Title: $posterTitle\n\n";
  $body .= "This is a re-transmission of the abstract document.\n\n";
  $body .= "Best regards,\n";
  $body .= "ICPM 2026 Submission System";

  $res = send_mail_with_attachment_ex($to, $subj, $body, $path, $name, "ICPM <ICPM@reg-sys.com>");
  ...
  // Log the attempt
  $logStatus = $res[0] ? 'RESEND_SUCCESS' : 'RESEND_FAIL';
  $logMsg = $res[0] ? "Resent successfully to $to" : "Failed to resend: " . $res[2];
  if (function_exists('log_submission')) {
      log_submission($authorEmail, $logStatus, $logMsg, $name, $refId);
  }
...
if ( isset( $_POST[ 'signup' ] ) )
{
  ...
  if ( $row > 0 )
  {
    ...
    if ($hasCoEmails) {
      $stmt = mysqli_prepare( $con, "insert into users(fname,nationality,coauth1name,coauth1nationality,coauth1email,coauth2name,coauth2nationality,coauth2email,coauth3name,coauth3nationality,coauth3email,coauth4name,coauth4nationality,coauth4email,coauth5name,coauth5nationality,coauth5email,email,profession,organization,postertitle,category,password,contactno,userip,companyref,paypalref,abstract_filename,abstract_mime,abstract_blob,supervisor_choice,supervisor_name,supervisor_nationality,supervisor_contact,supervisor_email) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)" );
      if ($stmt) {
        mysqli_stmt_bind_param($stmt, str_repeat('s', 35),
      ...
      $stmt = mysqli_prepare( $con, "insert into users(fname,nationality,coauth1name,coauth1nationality,coauth2name,coauth2nationality,coauth3name,coauth3nationality,coauth4name,coauth4nationality,coauth5name,coauth5nationality,email,profession,organization,postertitle,category,password,contactno,userip,companyref,paypalref,abstract_filename,abstract_mime,abstract_blob,supervisor_choice,supervisor_name,supervisor_nationality,supervisor_contact,supervisor_email) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)" );
      if ($stmt) {
    ...
    if (!$stmt) {
      $fallback = mysqli_prepare($con, "insert into users(fname,nationality,coauth1name,coauth1nationality,coauth1email,coauth2name,coauth2nationality,coauth2email,coauth3name,coauth3nationality,coauth3email,coauth4name,coauth4nationality,coauth4email,coauth5name,coauth5nationality,coauth5email,email,profession,organization,category,password,contactno,userip,companyref,paypalref,supervisor_choice,supervisor_name,supervisor_nationality,supervisor_contact,supervisor_email) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
      if ($fallback) {
        mysqli_stmt_bind_param($fallback, str_repeat('s', 31),
          $fname,$nationality,$coauth1name,$coauth1nationality,$coauth1email,
          $coauth2name,$coauth2nationality,$coauth2email,
          $coauth3name,$coauth3nationality,$coauth3email,
          $coauth4name,$coauth4nationality,$coauth4email,
    ...
    if ( $msg )
    {
      ...
      //   $useremail = $_POST[ 'email' ];
        ...
        $abres = send_mail_with_attachment_ex($committee, $cmsub, $cmbody, $abstractPath, $abstractName, "ICPM <ICPM@reg-sys.com>", $email);
        $abok = $abres[0];
        $usersub = "Abstract Submission Received - Ref #$refId";
        $userbody = "Thank you for your submission.\nReference: $refId\nExpected review timeline: 2–3 weeks.\n";
        $ures = send_mail_with_attachment_ex($email, $usersub, $userbody, $abstractPath, $abstractName, "ICPM <ICPM@reg-sys.com>");
        $uok = $ures[0];
...
<p><strong>Login:</strong> <a href="https://regsys.cloud/icpm2026/poster26/login.php" target="_blank" rel="noopener">https://regsys.cloud/icpm2026/poster26/login.php</a></p>
<hr style="border:none;border-top:1px solid #eee;margin:16px 0">
<div style="text-align:center">
<img src="' . $footerImg . '" alt="ICPM" width="200" height="78" style="display:inline-block">
</div>
