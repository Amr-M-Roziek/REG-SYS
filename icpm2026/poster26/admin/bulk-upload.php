...
if (isset($_POST['action']) && $_POST['action'] == 'process_batch') {
    ...
    while ($processed < $limit && ($data = fgetcsv($handle)) !== FALSE) {
        ...
        $isVisitor = false;
        foreach($cats as $c) {
            if(strcasecmp($c, 'Visitor') === 0) {
                $isVisitor = true;
                break;
            }
        }
        if($isVisitor && count($cats) > 1) {
             $errors[] = ['row' => $rowNum, 'email' => $email, 'reason' => 'Invalid category: Visitor cannot be combined with other roles'];
             continue;
        }
        ...
        mysqli_stmt_bind_param($insert, 'sssssssss', $fname, $lname, $nationality, $email, $profession, $organization, $category, $enc_password, $contact);
        ...
        <section class="wrapper">
            ...
            <div class="step-container">
                ...
                <div class="alert alert-info">
                    ...
                    - For multiple categories, separate with commas (e.g. "Speaker, Attendee").<br>
