...
## Forced Logout on Exit
- Overview:
  - When enabled, users are automatically logged out when leaving the `welcome.php` page.
  - Useful for kiosk or shared-device scenarios.
- Enable/Disable:
  - From `welcome.php`, use the toggle button “Enable Auto Logout on Exit” / “Disable Auto Logout on Exit”.
  - URL toggle: append `?auto_logout=1` to enable for the current session; use `?auto_logout=0` to disable.
- Behavior:
  - The page attaches a `beforeunload` and `pagehide` handler that calls the logout endpoint.
  - The request includes a session-bound logout token for validation.
- Security:
  - Logout requests are accepted only when the session token matches.
  - Sessions are fully cleared and the session cookie is deleted.
- Troubleshooting:
  - If the network drops during navigation, the logout beacon may fail. The session will persist; reloading the page allows manual logout.
