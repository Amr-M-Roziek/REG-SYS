...
if($con && isset($_POST['signup']))
{
  ...
  $organization = $_POST[ 'organization' ];
    ...
    if (is_array($categoryInput) && in_array('Visitor', $categoryInput) && count($categoryInput) > 1) {
        log_error($email, 'VALIDATION_FAIL', 'Visitor category combined with others');
        echo "<script>alert('Visitor category cannot be combined with other roles.');</script>";
        exit();
    }
...
Category: ' . htmlspecialchars($category, ENT_QUOTES, 'UTF-8') . '<br>
			...
			<div class="resp-tabs-container">
                ...
                <select type="text" class="text" name="profession" value="" required>
                  ...
                  <option value="Other">Other</option>
								...
								<p>Category (Hold Ctrl to select multiple)</p>
								<select class="text" name="category[]" multiple size="8" required>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (categorySelect) {
        categorySelect.addEventListener('change', function() {
            ...
            // Check if "Speaker" is selected along with other options (example validation)
            // The user requested validation to prevent invalid combinations.
            // Specifically for "Visitor" if it exists, but speaker.php might not have Visitor.
            // Let's assume the same Visitor logic if Visitor is an option, or general exclusivity.
            // Based on previous index.php logic:
