...
if ($action == 'save_template') {
    ...
    if (!empty($overrideEmail)) {
        if (!filter_var($overrideEmail, FILTER_VALIDATE_EMAIL)) {
            echo json_encode(['status' => 'error', 'message' => 'Invalid override email']);
            exit;
        }
    ...
    if ($user) {
        ...
        $user['verificationLink'] = $protocol . $_SERVER['HTTP_HOST'] . "/icpm2026/scientific/verify.php?id=" . $user['id'] . "&hash=" . $hash;
    ...
    $adminStmt = mysqli_prepare($con, "SELECT id FROM admin WHERE id=? AND password=? AND (access_scope='scientific' OR access_scope='both')");
    if ($adminStmt) {
    ...
    $pwRes = mysqli_query($con, "SELECT id, password FROM users WHERE (source_system='scientific' OR source_system='both')");
    $passwords = [];
    if ($pwRes) {
