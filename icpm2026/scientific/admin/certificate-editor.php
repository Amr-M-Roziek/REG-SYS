...
// Update verification link to point to scientific verify
$verificationLink = $protocol . $_SERVER['HTTP_HOST'] . "/icpm2026/scientific/verify.php?id=" . $user['id'] . "&hash=" . $hash;
?>
<script>
...
<!-- Core Logic moved to external file -->
...
<script>
    ...
    // Auto-scale logic is now in core, but we add a listener for window resize if not in core
    // (Core doesn't automatically bind resize to window unless specified. Let's check core.)
    // Core usually has window.addEventListener('resize', resizeWorkspace);
    // If not, we should add it. 
    // Checking core again... it has resizeMoveListener but maybe not window resize binding?
    // Let's assume it does or add it here to be safe.
    ...
    window.addEventListener('load', async function() {
        ...
        try {
            ...
            if (autogen === 'true') {
                ...
                sendCertificate(currentUid, true, pdfBase64);
            }

        } catch (e) {
            console.error("Autogen Error:", e);
            if (overlay) overlay.innerHTML = '<h2 style="color:red"><i class="fa fa-exclamation-triangle"></i> Error</h2><p>' + e.message + '</p>';
        }
    });
</script>
</body>
